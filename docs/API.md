...
# 1. 构建 UI 控制器（add_* 返回元素句柄，不再返回 self）
...
**推荐方式**：用 ElementHandle，SDK 自动填门名 + group_id（见上方示例）。

**旧式 4 参数**（仍兼容，适合无法拿到句柄的场景）：
...
s.connect(ui_idx, "Button is down", chip_idx, "fire",
          output_group=ctrl.element_group_id(0))   # Button 的 GUID
s.connect(ui_idx, "Value", chip_idx, "speed",
          output_group=ctrl.element_group_id(1))   # Slider 的 GUID
```

- `element_group_id(index_or_name)`：返回元素的稳定 GUID（`UIControllerBuilder` 方法）
- 不传 `output_group` 时连线仍能生成，但真机上有多个同类型元素时无法路由
